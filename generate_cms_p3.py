...
# P3 Model paths
YOLO_P3_PATH = BASE_DIR / "priority_3_augmentation" / "yolo_runs" / "sperm_cls_augmented" / "weights" / "best.pt"
EFFNET_P3_PATH = BASE_DIR / "priority_3_augmentation" / "weights" / "efficientnet_b0_augmented.pt"
...
def evaluate_yolo(class_names):
    print("\n--- Evaluating YOLOv8 Priority 3 (Augmentation) ---")
    if not YOLO_P3_PATH.exists():
        print("❌ YOLO P3 weights not found.")
        return None, None
        
    model = YOLO(str(YOLO_P3_PATH))
    all_preds = []
    all_labels = []
...
def evaluate_efficientnet(device, num_classes, class_names):
    print("\n--- Evaluating EfficientNet-B0 Priority 3 (Augmentation) ---")
    if not EFFNET_P3_PATH.exists():
        print("❌ EfficientNet P3 weights not found.")
        return None, None
        
    model = models.efficientnet_b0(weights=None)
    num_ftrs = model.classifier[1].in_features
    # Loading logic used for the multi-model pipeline since models might have dropout layers
    try:
        model.classifier = nn.Sequential(nn.Dropout(p=0.4, inplace=True), nn.Linear(num_ftrs, num_classes))
        model.load_state_dict(torch.load(EFFNET_P3_PATH, map_location=device))
    except RuntimeError:
        model = models.efficientnet_b0(weights=None)
...
def generate_cms():
    ...
    if y_true_yolo:
        plot_cm(y_true_yolo, y_pred_yolo, class_names, "YOLOv8 Priority 3 - Confusion Matrix", "cm_yolo_p3.png")
        
    # Evaluate EfficientNet P3
    y_true_eff, y_pred_eff = evaluate_efficientnet(device, len(class_names), class_names)
    if y_true_eff:
        plot_cm(y_true_eff, y_pred_eff, class_names, "EfficientNet-B0 Priority 3 - Confusion Matrix", "cm_effnet_p3.png")
        
    print(f"\n✅ All P3 matrices saved in: {OUTPUT_DIR}")
