...
def evaluate_efficientnet(device, num_classes, class_names):
    print("\n--- Evaluating EfficientNet-B0 Original ---")
    if not EFFICIENTNET_PATH.exists():
        print("❌ EfficientNet weights not found.")
        return None, None
    ...
    try:
        ...
        print(f"❌ Failed to load EffNet weights: {e}")
        return None, None
...
def generate_cms():
    ...
    if y_true_yolo:
        plot_cm(y_true_yolo, y_pred_yolo, class_names, "YOLOv8 Original - Confusion Matrix", "cm_yolo_orig.png")
        
    # Evaluate EfficientNet
    y_true_eff, y_pred_eff = evaluate_efficientnet(device, len(class_names), class_names)
    if y_true_eff:
        plot_cm(y_true_eff, y_pred_eff, class_names, "EfficientNet-B0 Original - Confusion Matrix", "cm_effnet_orig.png")
        
    print(f"\n✅ All matrices saved in: {OUTPUT_DIR}")
